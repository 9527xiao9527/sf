...
class SFHttpClient:
    ...
    def login_from_session(self, session_item: dict) -> tuple:
        """使用 box.dat 账号对象登录（loginapp + loginweb 两步）"""
        try:
...
# ==================== 账号执行 ====================
def run_account(account_url, index: int) -> Dict[str, Any]:
    """执行单个账号，account_url 可以是 dict（box.dat）或字符串（旧格式）"""
    logger = Logger()
    ...
    while retry_count < MAX_PROXY_RETRIES and not login_success:
        try:
            ...
            if isinstance(account_url, dict):
                success, user_id, phone = http.login_from_session(account_url)
            else:
