...
def load_accounts() -> list:
    """
    从环境变量 sfsydd 读取账号列表
    格式：JSON 数组字符串，每个元素是 {url, body, headers} 对象
    由 顺丰cookie.js 抓取后整体粘贴进青龙环境变量
    """
    env_value = os.getenv('sfsydd', '').strip()
    if not env_value:
        print('❌ 未找到环境变量 sfsydd，请检查配置')
        return []
    try:
        accounts = json.loads(env_value)
...
class SFHttpClient:
    ...
    def login_from_session(self, session_item: dict) -> tuple:
        """
        使用 box.dat 里的账号对象登录（两步：loginapp + loginweb）

        Args:
            session_item: {url, body, headers} 字典

        Returns:
            tuple: (是否成功, user_id, 手机号)
        """
        try:
            # 第一步：loginapp —— POST 到 ccsp 接口拿 sign
            headers = dict(session_item.get('headers', {}))
            headers.pop('Cookie', None)
            headers.pop('Content-Length', None)
            resp = self.session.post(
                session_item['url'],
                data=session_item.get('body', '{}'),
                headers=headers,
            ...
            sign = login_data.get('obj', {}).get('sign', '')
            if not sign:
                print(f'loginapp 未返回 sign，响应: {str(login_data)[:200]}')
                return False, '', ''

            # 从 body 里取 mobile
            try:
                phone = json.loads(session_item.get('body', '{}')).get('mobile', '')
            except Exception:
                phone = ''

            # 第二步：loginweb —— GET shareRedirect 拿 session cookie
            from urllib.parse import quote
            redirect_url = (
                f'https://mcs-mimp-web.sf-express.com/mcs-mimp/share/app/shareRedirect'
                f'?sign={quote(sign)}&source=SFAPP'
                f'&bizCode=647@RnlvejM1R3VTSVZ6d3BNaXJxRFpOUVVtQkp0ZnFpNDBKdytobm5TQWxMeHpVUXVrVzVGMHVmTU5BVFA1bXlwcw=='
            )
            self.session.get(redirect_url, headers=self.headers, timeout=PROXY_TIMEOUT)
            ...
            if phone:
                ...
                print('loginweb 后未获取到手机号 cookie')
                return False, '', ''
        except Exception as e:
            print(f'login_from_session 异常: {str(e)}')
            return False, '', ''
class TaskExecutor:
    """任务执行器"""
...
class AccountManager:
    ...
    def __init__(self, account_url, account_index: int, config: Config):
        # account_url 可以是字符串（旧格式）或 dict（box.dat session 对象）
        ...
        while retry_count < MAX_PROXY_RETRIES and not self.login_success:
            try:
                ...
                if isinstance(account_url, dict):
                    success, self.user_id, self.phone = self.http_client.login_from_session(account_url)
                else:
...
def main():
    ...
    # 从环境变量读取账号列表
    account_urls = load_accounts()
    if not account_urls:
        print(f"❌ 未找到账号，请在环境变量 sfsydd 中配置（JSON数组格式）")
        return
    print(f"📦 读取到 {len(account_urls)} 个账号")
